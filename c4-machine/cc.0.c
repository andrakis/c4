/*
 * cc.0.c - C4 C level 0 compiler
 * outputs assembly, and only understands basic c4 constructs.
 * used to compile cc.c
 * vim: set et ts=2:
 */

// originally Written by Robert Swierczek
//  with additions by Julian Thatcher
...
char *opcodes;
void setup_opcodes () {
    opcodes = "LEA ,IMM ,JMP ,JSR ,BZ  ,BNZ ,ENT ,ADJ ,LEV ,LI  ,LC  ,SI  ,SC  ,PSH ,"
              "OR  ,XOR ,AND ,EQ  ,NE  ,LT  ,GT  ,LE  ,GE  ,SHL ,SHR ,ADD ,SUB ,MUL ,DIV ,MOD ,"
              "OPEN,READ,CLOS,PRTF,MALC,FREE,MSET,MCMP,C4MA,EXIT,GOPC,GOCS,GSYM";
}

char *symbols () {
    return "char else enum if int return sizeof while "
           "open read close printf malloc free memset memcmp c4_main exit get_opcode get_opcodes get_symbols void main";
}
...
int main(int argc, char **argv)
{
  ...
  --argc; ++argv;
  if (argc < 1) { printf("usage: cc.0.c [-s] [-d] file ...\n"); return -1; }
  ...
  i = OPEN; while (i < OpMax) { next(); id[Class] = Sys; id[Type] = INT; id[Val] = i++; } // add library to symbol table
  ...
  // preamble
  printf(";; generated by cc.0.c from command: cc.0.c %s\n", *argv);
