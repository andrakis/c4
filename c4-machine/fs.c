...
int *fsnode_new (int type, int version, int isize, int icount, int iroot) {
	int sz, *node, *inodes;
...
int *fsnode_inode (int inode, int *fsnode) {
	return (int*)((char*)fsnode[FS_DATA] + (inode * (fsnode[FS_ISIZE] << 8)));
}
...
void dump_fs (int *fs) {
	printf(" FS [Type 0x%X] [Ver %d]    [ISize %d]      [ICount %d]  [IRoot %d]",
	       fs[FS_TYPE], fs[FS_VERSION], fs[FS_ISIZE] << 8, fs[FS_ICOUNT], fs[FS_IROOT]);
...
int *create_image (int type, int version, int isize, int icount, int iroot) {
	...
	if (!(fs = fsnode_new(type, version, isize, icount, iroot)))
		return 0;

	root = fsnode_iroot(fs);
	inode_write_str(iroot, ". 0\n.. 0\nREADME 1\n", FT_DIRECTORY, FP_READ | FP_EXECUTABLE, fs);
	file = iroot + 1;
...
int *read_image_fs (int fd) {
	int i_type, i_version, i_size, i_count, i_root;
	...
	if (!(fs = fsnode_new(i_type, i_version, i_size, i_count, i_root))) {
...
int main (int argc, char **argv) {
	...
	if (0) {
		g_image_fs = create_image(create_fstype, create_fsversion, create_isize, create_icount, 0);
		write_image(g_image, g_image_fs);
	}
	free_image(g_image_fs);
	if (!(g_image_fs = read_image(g_image))) {
		printf("Unable to read image: %s\n", g_image);
		return 1;
	}
	dump_fs(g_image_fs);
	dump_inode(fsnode_inode(0, g_image_fs));
