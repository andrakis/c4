// c4m.smt.c - C4 Multiload + Simultaneous MultiThreading (SMT)
...
//
// Usage:
//  gcc  -O2 -g -idirafter include -I . c4m.smt.c c4m_float.c -o c4m.smt -lm
//  ./c4m.smt [-c nn] [-sSd] <first file.c> [file n.c, ...] [-- arguments...]
// With c4m:
//  ./c4m c4m.smt.c4r [-c nn] [-sSd] <first file.c> [file n.c, ...] [-- arguments...]
// With load-c4r:
//  gcc -E -Iinclude -I. -DC4CC=1 -D__c4__=1 -D__C4CC__=1 -D__c4cc__=1 -C c4m.smt.c | ./c4cc -o c4m.smt.c4r -
//  ./c4m load-c4r.c -- c4m.smt.c4r [-c nn] [-sSd] <first file.c> [file n.c, ...] [-- arguments...]
//
// Parameters:
//  -c nn        Number of cores (default: 2)
...
// SMT Notes:
//  2026 / 01 / 03 - Started.
//  2026 / 01 / 04 (+3 hours) - First working version.
// Notes:
...
enum { // struct RegisterFile {
	// Static members
	RF_ID,  // Identifier (Low 16 bits: logical core)
	        //            (Hi  16 bits: logical processor)
	// Dynamic members
	RF_BP,
	RF_SP,
	RF_PC,
	RF_A,
	RF_C,    // Cycle
	RF_CI,   // Cycle interval
	RF_CH,   // Cycle interrupt handler
	RF_TH,   // Trap handler
	RF_SH,   // Signal handlers
	RF_PS,   // Pending Signal
	RF_PM,   // Protected mode (unused)
	RF__Sz
}; // };
...
//	   PUTC,PUTS,RALC,MCPY,STRC,ITH ,_OPC,_BLT,
...
void c4m_setup_builtins () {
	...
	//               "char else enum if int return sizeof while "
	//               "open read close printf malloc free memset memcmp "
	//			   "putchar puts realloc memcpy stacktrace "
...
int __opcode (char *name) {
	...
	if (!name) {
		printf("c4m.smt: invalid opcode given?\n");
		return -1;
	}
    if (!(ops = c4m_opcodes)) {
		printf("c4m.smt: lost opcodes?\n");
		return -1;
	}
    r = 0;
    while(r < INS_SIZE) {
	...
	printf("c4m.smt: opcode not found: '%s'\n", name);
    return -1;
}
...
void print_stacktrace (int *pc_orig, int *idmain, int *idmax, int *bp, int *sp) {
    ...
    while (!done) {
		// TODO: disabled
	    if (++cycles > 0x1FFFF) {
		  printf("c4m.smt: BUG ** print_stacktrace couldnt find function entry (1)\n");
		  return;
		}
        found = 0;
        comparisons = 0;
		...
		if (pc_orig - pc < distance_max) {
		  ...
		  printf("c4m.smt: BUG ** print_stacktrace couldnt find function entry (2)\n");
		  return;
		}
    }
}
...
int c4_time () {
	...
	if (c4_time_unavailable)
		return 0;
	// Only read time from timefile every N calls
	// TODO: offsets time too much, need to multiply result?
	if (0) {
	if (c4_time_last && ++c4_time_approx % C4_TIME_APPROX) {
		// printf("(cached %ld)\n", c4_time_approx);
		return c4_time_last;
	}
	}
	...
	if ((fd = open("/proc/uptime", 0)) < 0) {
		printf("c4m.smt: unable to open uptime file\n");
	...
	buf = c4_time_buf;
	if (!buf) {
		printf("c4m.smt: buffer went away\n");
		return 0;
	}
	r = read(fd, buf, C4_TIME_BUF_SZ);
	close(fd);
	if (r < 0) {
		printf("c4m.smt: read returned %d, buf: '%s' (0x%lx)\n", r, buf, buf);
		return 0;
	}
...
void trap (int type, int parameter, int *handler, int **_sp, int **_bp, int **_pc, int a, int mode) {
	...
	if (!handler) {
		printf("c4m.smt: missed a trap, no handler installed\n");
		return;
	}
...
int c4m_main(int argc, char **argv)
{
  ...
  int len;
  char *x;
  int *rf; // struct RegisterFile *
  int *rfp; // struct RegisterFile *
  int *sh;
  int procs, procmask, cores, coremask, corecycles;
  int proc, core, cc;
  ...
  procs = 16;
  cores = 16;
  coremask = 0x1; // only first core active
  procmask = 0x1; // only first processor active
  corecycles = 0x8;
  ...
  if ((i = __c4_signal_init())) { printf("c4m.smt: signal init failed with reason %d\n", i); return -1; }
  if (!(rf = malloc((i = sizeof(int) * procs * cores * RF__Sz)))) { printf("c4m.smt: failed to allocate register file of size %ld bytes\n", i); return -1; }
  memset(rf,   0, i);
  memset(sym,  0, poolsz);
  memset(e,    0, poolsz);
  ...
  // parse declarations
  if (verb) printf("c4m.smt: compile...\n");
  line = 1;
  next();
  while (tk) {
    bt = INT; // basetype
  ...
  if (verb) printf("c4m.smt: prepare...\n");
  // setup stack
  ...
  *--sp = (int)t; // printf("//sp(0x%X) = t (0x%X)\n", sp, t);
  ...
  rfp = rf; // first processor, first core
  rfp[RF_BP] = (int)bp;
  rfp[RF_SP] = (int)sp;
  rfp[RF_PC] = (int)pc;
  ...
  while (run) {
	...
	rfp  = rf;
	while (proc++ < procs) {
		if (!(procmask & (1 << (proc - 1)))) {
			// Skip this proc
			rfp = rfp + (RF__Sz * cores);
			continue;
		}
		while (core++ < cores) {
			if (!(coremask & (1 << (core - 1)))) {
				// Skip this core
				rfp = rfp + RF__Sz;
				continue;
			}
			...
			while (cc--) {
				...
				if (rfp[RF_CI] && !(rfp[RF_C] % rfp[RF_CI])) {
					...
					trap(TRAP_HARD_IRQ, HIRQ_CYCLE, (int *)rfp[RF_CH], &sp, &bp, &pc, a, mode);
					// Disable cycle interrupt and set unprotected mode
					// cycle_interrupt_interval = 0;
					rfp[RF_CI] = 0;
					// mode = MODE_UNPROTECTED;
					// rfp[RH_PM] = MODE_UNPROTECTED;
				// Check for pending signals from the signal handler
				// TODO: in SMT mode, the pending signal only gets delivered to first proc and core
				} else if (pending_signal && proc == 1 && core == 1) {
				// } else if (rfp[RF_PS]) {
					// TODO: sometimes crashes here, as if signal_handlers[pending_signal] points to code that has been
					//       deallocated.
					// printf("c4m.smt: trapping pending signal %d\n", pending_signal);
					trap(TRAP_SIGNAL, pending_signal, (int *)signal_handlers[pending_signal], &sp, &bp, &pc, a, mode);
					// Disable cycle interrupt and set unprotected mode
					// mode = MODE_UNPROTECTED;
					// cycle_interrupt_interval = 0;
					pending_signal = 0;
					// rfp[RF_PM] = MODE_UNPROTECTED;
					rfp[RF_CI] = rfp[RF_PS] = 0;
				}
				...
				if (i == OPCD) {
					...
					if (i <= ADJ) {
						...
						trap(TRAP_OPV, i, (int *)rfp[RF_TH], &sp, &bp, &pc, a, mode);
						// Disable cycle interrupt and set unprotected mode
						// cycle_interrupt_interval = 0;
						rfp[RF_CI] = 0;
						// mode = MODE_UNPROTECTED;
			...
			//		}
				  ...
				  // Trigger a trap
				  trap(sp[1], sp[0], (int *)rfp[RF_TH], &sp, &bp, &pc, a, mode);
				  // Disable cycle interrupt and set unprotected mode
			//	  cycle_interrupt_interval = 0;
			//	  mode = MODE_UNPROTECTED;
				} else if (i == DBG) {
					// DBG - signal the debugger
					trap(TRAP_DEBUG, 0, (int *)rfp[RF_TH], &sp, &bp, &pc, a, mode);
					// Disable cycle interrupt and set unprotected mode
			//		mode = MODE_UNPROTECTED;
				} else if (i == C4IV) {
					// C4 Invoke
					...
					*c4_invoke_stub_addr = *sp;
					...
					if (0) {
						...
						printf("c4m.smt: invoke only supported under c4.\n");
						a = 0;
					}
#endif
				} else if (i == FLT) {
					if (has_float()) a = do_float(sp);
						...
						trap(TRAP_ILLOP, FLT, (int *)rfp[RF_TH], &sp, &bp, &pc, a, mode);
					}
				} else {
			//        if (trap_handler == 0) {
			//            printf("unknown instruction = %d! cycle = %d\n", i, cycle); status = -1; run = 0;
			//        } else {
						// printf("c4m.smt: illegal opcode %d, invoking trap handler 0x%X\n", i, trap_handler);
						trap(TRAP_ILLOP, i, (int *)rfp[RF_TH], &sp, &bp, &pc, a, mode);
			//        }
				}
			}
