...
// the plain-c4 subset. Appending is free; inserting is not. SMP
// opcodes land at 66+ in a later stage.
enum {
...
// __c4_info() bits. 0x100 and 0x200 are reserved here for C4I_SMP and
// C4I_CISC, claimed when those stages land -- a guest must be able to
// ask before it dares execute an opcode c4m does not have.
enum {
    C4I_NONE  = 0x0,
    C4I_C4    = 0x1,
    C4I_C4M   = 0x2,
...
// CPU states. Only OFF and RUN exist in stage 0; WAIT and HALT arrive
// with CWAI and CPUH.
