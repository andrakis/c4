// body.h -- the driver, shared by both builds.
//
// c4sc-host.c INCLUDES the generated units, for the native binary the
// differential tests run. c4sc-main.c DECLARES them instead and is
// compiled by c4lc -c into an object that c4rlink joins to the seven
// generated objects, which is how the compiler becomes a .c4r. The
// modes below are identical either way; only how the units arrive
// differs.
...
int sc_compile (int argc, char **argv) {
	...
	// The size line comes BEFORE the write, because under bare c4m there
	// is no write syscall and the write is the only part that cannot
	// happen -- the compile itself has. Reporting first makes a hosted
	// run comparable with a native one on the number that matters.
	printf(";; c4lc: %s - %d bytes - %s\n", in, out[CELL_B], outname);
	r = sc_call_named("file:write", cons(mk_string(outname), cons(out, 0)));
	if (c4sp_err || cell_is_false(r)) { printf("c4sc: write failed\n"); return 1; }
	return 0;
}
